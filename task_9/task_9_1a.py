...
# Функция должна возвращать список всех портов в режиме access с конфигурацией на основе шаблона
# access_mode_template и шаблона port_security_template, если он был передан. В конце строк в списке не должно быть символа перевода строки.
...
def generate_access_config(intf_access_conf, access_mode_templates, psecurity=None):
    access_config = []

    for intf, vlan in intf_access_conf.items():
        access_config.append(f"interface {intf}")
        for command in access_mode_templates:
