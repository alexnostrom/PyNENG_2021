...
# и надо понимать, что с ним делать. Поэтому в соответствии каждому
# порту стоит список и первый (нулевой) элемент списка указывает
...
# То есть, если в словаре trunk будут другие номера интерфейсов, код должен работать.
...
for intf, vlan in trunk.items():
    ...
    for command in trunk_template:
        if command.endswith('allowed vlan'):
            print(f' {trunk_commands[trunk[intf][0]]} {",".join(vlan[1:])}')
        else:
            print(f' {command}')
