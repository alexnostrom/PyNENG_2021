...
# на стандартный поток вывода, должна возвращаться соответствующая конфигурация access или trunk (
...
# Подсказка: Подводящим к этому заданию было задание 5.1.
# Чтобы было легче решить это задание, можно посмотреть на задание
# 5.1 и разобраться как там получилось вывести разную информацию в зависимости от ввода пользователя.
...
if interface_mode.lower() == 'access':
    ...
    vlans_quantity = input("Введите номер влан(ов): ")
    ...
    trunk_template = [
        ...
        "switchport trunk allowed vlan {}".format(vlans_quantity)
    ]
